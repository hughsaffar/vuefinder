...
const Us = (t, e) => t.indexOf(e) >= 0, Mt = (t, e) => t.concat(e), pe = (t, e, n) => (!Lt(e) && Yn(e) ? Array.prototype.push.apply(t, e) : t.push(e), t), rt = (t) => Array.from(t || []), Xn = (t) => qe(t) ? t : !Lt(t) && Yn(t) ? rt(t) : [t], Mn = (t) => !!t && !t.length, Dn = (t) => rt(new Set(t)), Ve = (t, e, n) => {
  ...
  const v = function(y) {
    ...
    const y = rt(arguments), V = Fe(n) ? n() : n;
    if (Ue(V) && V >= 0) {
      const R = Fe(a) ? a() : a, w = Ue(R) && R >= 0, D = V > 0 ? Zt : Gn, q = V > 0 ? Tn : Ns, E = p(y) || y, A = v.bind(0, E);
      let C;
      f(), s && !u ? (A(), u = !0, C = D(() => u = void 0, V)) : (C = D(A, V), w && !i && (i = Zt(m, R))), f = () => q(C), d = l = E;
    } else
      v(y);
  };
  return h.m = m, h;
...
const na = () => {
  const t = (w, D, q) => {
    ...
    const q = ts(w, jt);
    try {
      D = Je(w, "scrollbar-width") === "none" || Je(w, "display", "::-webkit-scrollbar") === "none";
    } catch {
    }
    return q(), D;
  ...
  const { j: c, X: i, Y: d, W: l, J: u, G: f } = a || {}, v = Vn(() => s && n(!0), {
    _: 33,
    p: 99
  }), [p, m] = la(t, v, d), h = c || [], $ = i || [], y = Mt(h, $), V = (R, w) => {
    if (!Mn(w)) {
      const D = u || dt, q = f || dt, S = [], E = [];
      let A = !1, C = !1;
      if (de(w, (x) => {
        const { attributeName: L, target: B, type: k, oldValue: I, addedNodes: N, removedNodes: ee } = x, se = k === "attributes", oe = k === "childList", Z = t === B, T = se && L, j = T && Zn(B, L || ""), F = Lt(j) ? j : null, G = T && I !== F, H = Us($, L) && G;
        if (e && (oe || !Z)) {
          const W = se && G, z = W && l && Ln(B, l), ne = (z ? !D(B, L, I, F) : !se || W) && !q(x, !!z, t, a);
          de(N, (ve) => pe(S, ve)), de(ee, (ve) => pe(S, ve)), C = C || ne;
        }
        !e && Z && G && !D(B, L, I, F) && (pe(E, L), A = A || H);
      }), m((x) => Dn(S).reduce((L, B) => (pe(L, oo(x, B)), Ln(B, x) ? pe(L, B) : L), [])), e)
        return !R && C && n(!1), [!1];
      if (!Mn(E) || A) {
        const x = [Dn(E), A];
  ...
  let s, c, i, d, l, u;
  const f = `[${st}]`, v = `[${Xe}]`, p = ["id", "class", "style", "open", "wrap", "cols", "rows"], { vt: m, gt: h, ot: $, ht: y, bt: V, nt: O, wt: R, yt: w, St: D, Ot: q } = t, S = (H) => Je(H, "direction") === "rtl", E = {
    $t: !1,
    ct: S(m)
  }, A = je(), C = Ot(Co), [x] = Te({
    i: Js,
    ...
    const H = C && C.tt(t, e, E, A, n).ut, z = !(R && O) && es(h, st, Xt), Y = !O && w(Nr), ne = Y && De(y), ve = ne && q(), Ce = D(po, z), be = Y && H && H()[0], ke = on($), te = ns($);
  ...
  return [() => {
    F && F.observe(h);
    const H = Z && Z(), W = se && se(), z = T(), Y = A.L((ne) => {
      ne ? L({
        zt: ne
      }) : G();
    });
  ...
  const s = "--os-viewport-percent", c = "--os-scroll-percent", i = "--os-scroll-direction", { U: d } = je(), { scrollbars: l } = d(), { slot: u } = l, { vt: f, gt: v, ot: p, Tt: m, ht: h, wt: $, nt: y } = e, { scrollbars: V } = m ? {} : t, { slot: O } = V || {}, R = [], w = [], D = [], q = yo([f, v, p], () => y && $ ? f : v, u, O), S = (T) => {
    if (Kt) {
      const j = new Kt({
        source: h,
        axis: T
    ...
    const { Rt: T, Vt: j } = n, F = (G, H) => eo(0, 1, G / (G + H) || 0);
    return {
      x: F(j.x, T.x),
      y: F(j.y, T.y)
    };
    ...
    const T = A(), j = (F) => (G) => [G.kt, {
      [s]: Hn(F) + ""
    }];
    x(w, j(T.x)), x(D, j(T.y));
  }, k = () => {
    if (!Kt) {
      const { Lt: T } = n, j = bs(T, De(h)), F = (G) => (H) => [H.kt, {
        [c]: Hn(G) + ""
      }];
      x(w, F(j.x)), x(D, F(j.y));
    }
  }, I = () => {
    const { Lt: T } = n, j = gs(T), F = (G) => (H) => [H.kt, {
      [i]: G ? "0" : "1"
    }];
    x(w, F(j.x)), x(D, F(j.y));
  }, N = () => {
    if (y && !$) {
      const { Rt: T, Lt: j } = n, F = gs(j), G = bs(j, De(h)), H = (W) => {
    ...
    const j = T ? "x" : "y", G = pt(`${Le} ${T ? Wr : Yr}`), H = pt(go), W = pt(os), z = {
      kt: G,
      Ut: H,
      Pt: W
    }, Y = E[j];
    return pe(T ? w : D, z), pe(R, [Me(G, H), Me(H, W), X(bt, G), Y && Y.Mt(z), a(z, L, T)]), z;
  }, se = X(ee, !0), oe = X(ee, !1), Z = () => (Me(q, w[0].kt), Me(q, D[0].kt), X(Ve, R));
  return se(), oe(), [{
    Nt: B,
    qt: k,
    Bt: I,
  ...
  const { gt: d, ot: l, nt: u, ht: f, Kt: v, Ot: p } = e, { kt: m, Ut: h, Pt: $ } = s, [y, V] = ft(333), [O, R] = ft(444), w = (S) => {
    ...
    const S = "pointerup pointercancel lostpointercapture", E = `client${i ? "X" : "Y"}`, A = i ? dn : un, C = i ? "left" : "top", x = i ? "w" : "h", L = i ? "x" : "y", B = (I, N) => (ee) => {
      const { Rt: se } = n, oe = ht(h)[x] - ht($)[x], T = N * ee / oe * se[L];
      Ne(f, {
        [L]: I + T
      });
    }, k = [];
    return fe(h, "pointerdown", (I) => {
      const N = mt(I.target, `.${os}`) === $, ee = N ? $ : h, se = t.scrollbars, { button: oe, isPrimary: Z, pointerType: T } = I, { pointers: j } = se;
      if (oe === 0 && Z && se[N ? "dragScroll" : "clickScroll"] && (j || []).includes(T)) {
        Ve(k), R();
        const G = !N && I.shiftKey, H = X($n, $), W = X($n, h), z = (ie, le) => (ie || H())[C] - (le || W())[C], Y = An($n(f)[A]) / ht(f)[x] || 1, ne = B(De(f)[L], 1 / Y), ve = I[E], Ce = H(), be = W(), ke = Ce[A], te = z(Ce, be) + ke / 2, we = ve - be[C], ye = N ? 0 : we - te, xe = (ie) => {
        ...
        }, fe(v, S, xe), fe(v, "selectstart", (ie) => Bn(ie), {
          H: !1
        }), fe(h, S, xe), fe(h, "pointermove", (ie) => {
          const le = ie[E] - ve;
          (N || G) && ne(ye + le);
        })];
  ...
  return X(Ve, [fe($, "pointermove pointerleave", a), fe(m, "pointerenter", () => {
    ...
    const S = Fn();
    (fs(S, Xe) || fs(S, st) || S === document.body) && Zt(X(In, l), 25);
  }), fe(m, "wheel", (S) => {
    const { deltaX: E, deltaY: A, deltaMode: C } = S;
    q && C === 0 && Dt(m) === d && w({
      x: E,
      y: A
    }), q = !1, c(Ts, !0), y(() => {
  ...
  const m = (T) => T.pointerType === "mouse", [h, $] = ft(), [y, V] = ft(100), [O, R] = ft(100), [w, D] = ft(() => p), [q, S] = pa(t, s, a, ha(e, s, a, (T) => m(T) && ee())), { gt: E, Qt: A, wt: C } = s, { jt: x, Nt: L, qt: B, Bt: k, Ft: I } = q, N = (T, j) => {
    if (D(), T)
      x(As);
    else {
      const F = X(x, As, !0);
    ...
    m(T) && (i = l, l && N(!0));
  }, Z = [D, V, R, $, () => v(), fe(E, "pointerover", oe, {
    A: !0
  }), fe(E, "pointerenter", oe), fe(E, "pointerleave", (T) => {
    m(T) && (i = !1, l && N(!1));
  }), fe(E, "pointermove", (T) => {
    m(T) && d && ee();
  }), fe(A, "scroll", (T) => {
  ...
  return [() => X(Ve, pe(Z, S())), ({ It: T, Dt: j, Zt: F, tn: G }) => {
    const { nn: H, sn: W, en: z, cn: Y } = G || {}, { Ct: ne, dt: ve } = F || {}, { ct: Ce } = n, { M: be } = je(), { K: ke, rn: te } = a, [we, ye] = T("showNativeOverlaidScrollbars"), [xe, Se] = T("scrollbars.theme"), [me, ie] = T("scrollbars.visibility"), [le, Ae] = T("scrollbars.autoHide"), [He, Qe] = T("scrollbars.autoHideSuspend"), [St] = T("scrollbars.autoHideDelay"), [Ft, at] = T("scrollbars.dragScroll"), [Ct, pn] = T("scrollbars.clickScroll"), [Ht, hn] = T("overflow"), gn = ve && !j, Re = te.x || te.y, bn = H || W || Y || ne || j, Rt = z || ie || hn, wn = we && be.x && be.y, lt = (Et, _t, Bt) => {
      const At = Et.includes(gt) && (me === tt || me === "auto" && _t === gt);
      return x(Xr, At, Bt), At;
    };
    if (p = St, gn && (He && Re ? (se(!1), v(), O(() => {
      v = fe(A, "scroll", X(se, !0), {
        A: !0
      });
    ...
    bn && (B(), L(), I(), Y && k(), x(Es, !te.x, !0), x(Es, !te.y, !1), x(Kr, Ce && !C));
  }, {}, q];
}, ba = (t) => {
  const e = je(), { U: n, R: a } = e, { elements: s } = n(), { padding: c, viewport: i, content: d } = s, l = nn(t), u = l ? {} : t, { elements: f } = u, { padding: v, viewport: p, content: m } = f || {}, h = l ? t : u.target, $ = ro(h), y = h.ownerDocument, V = y.documentElement, O = () => y.defaultView || Ee, R = X(sa, [h]), w = X(yo, [h]), D = X(pt, ""), q = X(R, D, i), S = X(w, D, d), E = (te) => {
    const we = ht(te), ye = on(te), xe = Je(te, Ys), Se = Je(te, Xs);
    return ye.w - we.w > 0 && !wt(xe) || ye.h - we.h > 0 && !wt(Se);
  }, A = q(p), C = A === h, x = C && $, L = !C && S(m), B = !C && A === L, k = x ? V : A, I = x ? k : h, N = !C && w(D, c, v), ee = !B && L, se = [ee, k, N, I].map((te) => nn(te) && !Dt(te) && te), oe = (te) => te && Us(se, te), Z = !oe(k) && E(k) ? k : h, T = x ? V : k, F = {
    vt: h,
    gt: I,
    ot: k,
    ln: N,
  ...
  return [F, () => {
    ...
    return Ye(H, st, C ? "" : Rr), Ye(W, Un, ""), Ye(z, Xe, ""), Ye(Y, Ss, ""), C || (Ye(z, Se, me || "-1"), $ && Ye(V, xs, "")), Me(Ce, ve), Me(H, W), Me(W || H, !C && z), Me(z, Y), pe(ne, [ie, () => {
      const le = Fn(), Ae = oe(z), He = Ae && le === z ? G : le, Qe = xe(He);
      Be(W, Un), Be(Y, Ss), Be(z, Xe), $ && Be(V, xs), me ? Ye(z, Se, me) : Be(z, Se), oe(Y) && ye(Y), Ae && ye(z), oe(W) && ye(W), In(He), Qe();
    }]), a && !C && (Qn(z, Xe, ho), pe(ne, X(Be, z, Xe))), In(!C && $ && we === G && te.top === te ? z : we), ie(), ve = 0, be;
  }, be];
  ...
  return ({ It: d, Zt: l, _n: u, Dt: f }) => {
    let [v, p] = i(f);
    const { R: m } = je(), { ft: h, Ht: $, Ct: y } = l || {}, { ct: V } = u, [O, R] = d("paddingAbsolute");
    (h || p || (f || $)) && ([v, p] = c(f));
    const D = !a && (R || y || p);
    if (D) {
      const q = !O || !e && !m, S = v.r + v.l, E = v.t + v.b, A = {
        [Ks]: q && !V ? -S : 0,
        [Ws]: q ? -E : 0,
        [Gs]: q && V ? -S : 0,
        top: q ? -v.t : 0,
        right: q ? V ? -v.r : "auto" : 0,
        left: q ? V ? "auto" : -v.l : 0,
        [dn]: q && `calc(100% + ${S}px)`
      }, C = {
        [qs]: q ? v.t : 0,
  ...
  }, [w, D] = Te(y, X(ns, c)), [q, S] = Te(y, X(on, c)), [E, A] = Te(y), [C] = Te(V), [x, L] = Te(y), [B] = Te(V), [k] = Te({
    i: (Z, T) => _n(Z, T, $),
    o: {}
  }, () => Or(c) ? Je(c, $) : {}), [I, N] = Te({
    i: (Z, T) => Wt(Z.D, T.D) && Wt(Z.T, T.T),
    o: fo()
  }, () => {
    O(!0);
    const Z = De(l), T = f(qr, !0), j = fe(d, gt, (z) => {
      const Y = De(l);
  ...
  }), ee = Ot(Co), se = (Z, T) => `${T ? Br : Ir}${Cr(Z)}`, oe = (Z) => {
    const T = (F) => [tt, it, gt].map((G) => se(G, F)), j = T(!0).concat(T()).join(" ");
    f(j), f(Pe(Z).map((F) => se(Z[F], F === "x")).join(" "), !0);
  };
  return ({ It: Z, Zt: T, _n: j, Dt: F }, { fn: G }) => {
    const { ft: H, Ht: W, Ct: z, dt: Y, zt: ne } = T || {}, ve = ee && ee.tt(t, e, j, n, Z), { it: Ce, ut: be, _t: ke } = ve || {}, [te, we] = ua(Z, n), [ye, xe] = Z("overflow"), Se = wt(ye.x), me = wt(ye.y), ie = !0;
    let le = D(F), Ae = S(F), He = A(F), Qe = L(F);
    we && p && f(ho, !te);
    {
      es(a, st, Xt) && O(!0);
      ...
      cs && cs(), Qe = x(ds), He = E(R(Xo, ds), F);
    }
    const [St, Ft] = Qe, [at, Ct] = He, [pn, Ht] = Ae, [hn, gn] = le, [Re, bn] = C({
      x: at.w > 0,
      y: at.h > 0
  ...
  return [n, (m) => {
    const h = {}, y = v && De(i), V = y && l();
    return de(p, (O) => {
      re(h, O(m, h) || {});
    }), Ne(i, y), V && V(), !d && Ne(c, 0), h;
  }, s, e, a];
}, xa = (t, e, n, a, s) => {
  let c = !1;
  const i = ks(e, {}), [d, l, u, f, v] = ka(t), [p, m, h] = ma(f, u, i, (w) => {
    R({}, w);
  }), [$, y, , V] = ga(t, e, h, u, f, s), O = (w) => Pe(w).some((D) => !!w[D]), R = (w, D) => {
    if (n())
      return !1;
    const { vn: q, Dt: S, At: E, gn: A } = w, C = q || {}, x = !!S || !c, L = {
      It: ks(e, C, x),
      vn: C,
      Dt: x
    };
    ...
    const I = O(B), N = O(k), ee = I || N || !Jn(C) || x;
    return c = !0, ee && a(w, {
      Zt: B,
      tn: k
    }), ee;
  ...
  return [() => {
    const { an: w, ht: D, Ot: q } = f, S = De(w), E = [p(), d(), $()], A = q();
    return Ne(D, S), A(), X(Ve, E);
  }, R, () => ({
    hn: h,
    bn: u
  }), {
  ...
  if (e && !i) {
    let d = !1;
    const l = [], u = {}, f = (C) => {
      const x = Qs(C), L = Ot(ca);
      return L ? L(x, !0) : x;
    }, v = re({}, a(), f(e)), [p, m, h] = Nn(), [$, y, V] = Nn(n), O = (C, x) => {
      V(C, x), h(C, x);
    }, [R, w, D, q, S] = xa(t, v, () => d, ({ vn: C, Dt: x }, { Zt: L, tn: B }) => {
      const { ft: k, Ct: I, xt: N, Ht: ee, Et: se, dt: oe } = L, { nn: Z, sn: T, en: j, cn: F } = B;
      O("updated", [A, {
        updateHints: {
          sizeChanged: !!k,
          directionChanged: !!I,
      ...
      aa(c), Ve(l), d = !0, O("destroyed", [A, C]), m(), y();
    }, A = {
      options(C, x) {
        if (C) {
          const L = x ? a() : {}, B = mo(v, re(L, f(C)));
          Jn(B) || (re(v, B), w({
            vn: B
          }));
        }
      ...
      state() {
        const { hn: C, bn: x } = D(), { ct: L } = C, { Vt: B, Rt: k, K: I, rn: N, ln: ee, dn: se, Lt: oe } = x;
        return re({}, {
          overflowEdge: B,
          overflowAmount: k,
          overflowStyle: I,
      ...
      elements() {
        const { vt: C, gt: x, ln: L, ot: B, bt: k, ht: I, Qt: N } = q.wn, { Xt: ee, Gt: se } = q.yn, oe = (T) => {
          const { Pt: j, Ut: F, kt: G } = T;
          return {
            scrollbar: G,
            track: F,
            handle: j
    ...
    return pe(l, [S]), ra(c, A), So(ko, ze, [A, p, u]), oa(q.wn.wt, !s && t.cancel) ? (E(!0), A) : (pe(l, R()), O("initialized", [A]), A.update(), A);
  }
  return i;
};
ze.plugin = (t) => {
...
function Sa() {
  ...
  function m() {
    ...
    }), t.subscribe("DS:start:pre", ({ items: w, event: D, isDragging: q }) => {
      var S;
      if (q)
        t.Interaction._reset(D);
      else {
        a.value = !1;
        const E = ((S = e.value) == null ? void 0 : S.offsetWidth) - D.offsetX, A = e.value.offsetHeight - D.offsetY;
        E < 15 && A < 15 && t.Interaction._reset(D), D.target.classList.contains("os-scrollbar-handle") && t.Interaction._reset(D);
      }
  ...
  const h = () => ct(() => {
    ...
    const w = c().map((D) => D.path);
    l(), t.setSettings({
      selectables: document.getElementsByClassName("vf-item-" + n)
    }), t.addSelection(
      t.getSelectables().filter((D) => w.includes(JSON.parse(D.dataset.item).path))
    ), $(), O();
  }), V = (w) => {
    u.value = w, t.subscribe("DS:end", ({ items: D, event: q, isDragging: S }) => {
      s.value = D.map((E) => JSON.parse(E.dataset.item)), w(D.map((E) => JSON.parse(E.dataset.item)));
    });
  }, O = () => {
    var w, D;
    f.value && (((w = e.value) == null ? void 0 : w.getBoundingClientRect().height) < e.value.scrollHeight ? (v.value.style.height = ((D = e.value) == null ? void 0 : D.scrollHeight) + "px", v.value.style.display = "block") : (v.value.style.height = "100%", v.value.style.display = "none"));
  }, R = (w) => {
    if (!f.value)
      return;
    const { scrollOffsetElement: D } = f.value.elements();
    D.scrollTo(
      {
...
function Ca(t, e) {
  ...
  function p() {
    ...
    w.length === 0 && (s.value = []), w.replace(n.value + "://", "").split("/").forEach(function(S) {
      O.push(S), O.join("/") !== "" && R.push({
        basename: S,
        name: S,
        path: n.value + "://" + O.join("/"),
        type: "dir"
...
const Ua = { render: Na }, qa = { class: "vuefinder__modal-header" }, Pa = { class: "vuefinder__modal-header__icon-container" }, za = {
  ...
  key: 1,
  class: "vuefinder__about-modal__tab-content"
}, el = { class: "vuefinder__about-modal__description" }, tl = { class: "vuefinder__about-modal__settings" }, nl = { class: "vuefinder__about-modal__setting flex" }, sl = { class: "vuefinder__about-modal__setting-input" }, ol = { class: "vuefinder__about-modal__setting-label" }, rl = {
  for: "metric_unit",
  class: "vuefinder__about-modal__label"
}, al = { class: "vuefinder__about-modal__setting flex" }, ll = { class: "vuefinder__about-modal__setting-input" }, il = { class: "vuefinder__about-modal__setting-label" }, cl = {
  for: "large_icons",
  class: "vuefinder__about-modal__label"
                    ...
                    r("div", yl, [
                      _e(r("select", {
                        ...
                        r("optgroup", {
                          ...
                          (_(!0), g(he, null, ge(V.value, (w, D) => (_(), g("option", { value: D }, b(w), 9, kl))), 256))
                        ], 8, $l)
                      ], 544), [
                        [En, o(e).theme.value]
                      ]),
                    ...
                    r("div", El, [
                      _e(r("select", {
                        ...
                        r("optgroup", {
                          ...
                          (_(!0), g(he, null, ge(o(y), (w, D) => (_(), g("option", { value: D }, b(w), 9, Tl))), 256))
                        ], 8, Al)
                      ], 512), [
                        [En, o(e).i18n.locale]
                      ]),
...
function Vi(t) {
  ...
  $e(() => {
    var n;
    (n = t.root) == null || n.addEventListener("keydown", e);
  }), jn(() => {
    var n;
    (n = t.root) == null || n.removeEventListener("keydown", e);
  });
}
const Li = {
  xmlns: "http://www.w3.org/2000/svg",
...
const Oo = { render: Qi }, ec = { class: "vuefinder__upload-modal__content" }, tc = {
  ...
  setup(t) {
    ...
    function E(L) {
      h.value || (y.removeFile(L.id, "removed-by-user"), p.value.splice(V(L.id), 1));
    }
    function A(L) {
      if (!h.value) {
    ...
    return $e(async () => {
      y = new cr({
        ...
        const N = p.value[V(k.id)];
        E(N), m.value = I.message;
      }), y.on("upload", () => {
        const k = x();
        y.setMeta({ ...k.body });
        ...
        p.value[V(k.id)].percent = `${N}%`;
      }), y.on("upload-success", (k) => {
        const I = p.value[V(k.id)];
          ...
          r("div", ec, [
            ...
            r("div", {
              ...
              r("button", {
                ...
                class: "vf-btn vf-btn-secondary",
                disabled: h.value,
                onClick: B[0] || (B[0] = (k) => A(!1))
              }, b(o(n)("Clear all")), 9, sc),
              r("button", {
                type: "button",
                class: "vf-btn vf-btn-secondary",
                disabled: h.value,
                onClick: B[1] || (B[1] = (k) => A(!0))
...
const Nd = { render: Id }, Ud = { class: "vuefinder__move-modal__content" }, qd = { class: "vuefinder__move-modal__description" }, Pd = { class: "vuefinder__move-modal__files vf-scrollbar" }, zd = { class: "vuefinder__move-modal__file" }, jd = {
  key: 0,
  ...
  setup(t) {
    ...
    Oe(i, (S) => {
      ...
      e.fs.limitBreadcrumbItems(x), ct(() => {
        for (let B = E.length - 1; B >= 0 && !(A + E[B].offsetWidth > i.value - 40); B--)
          A += parseInt(E[B].offsetWidth, 10), C++;
        C < L && (C = L), C > x && (C = x), e.fs.limitBreadcrumbItems(C);
      });
    });
    const d = () => {
      var S;
      i.value = (S = c.value) == null ? void 0 : S.offsetWidth;
    };
    $e(() => {
      new ResizeObserver(d).observe(c.value);
    });
    const l = (S, E = null) => {
      S.preventDefault(), a.isDraggingRef.value = !1, v(S), E ?? (E = e.fs.hiddenBreadcrumbs.length - 1);
      let A = JSON.parse(S.dataTransfer.getData("items"));
      ...
      S.preventDefault(), a.isDraggingRef.value = !1, v(S), E ?? (E = e.fs.breadcrumbs.length - 2);
      let A = JSON.parse(S.dataTransfer.getData("items"));
      if (A.find((C) => C.storage !== e.fs.adapter)) {
        alert("Moving items between different storages is not supported yet.");
        return;
      ...
      e.emitter.emit("vf-fetch", { params: { q: "index", adapter: e.fs.adapter, path: S.path } }), e.fs.toggleHiddenBreadcrumbs(!1);
    }, $ = () => {
      e.fs.showHiddenBreadcrumbs && e.fs.toggleHiddenBreadcrumbs(!1);
    }, y = {
      mounted(S, E, A, C) {
        S.clickOutsideEvent = function(x) {
          S === x.target || S.contains(x.target) || E.value();
        }, document.body.addEventListener("click", S.clickOutsideEvent);
      },
      beforeUnmount(S, E, A, C) {
        document.body.removeEventListener("click", S.clickOutsideEvent);
      }
    }, V = () => {
      e.showTreeView = !e.showTreeView;
    };
    Oe(() => e.showTreeView, (S, E) => {
      S !== E && s("show-tree-view", S);
    });
    const O = M(null), R = () => {
    ...
    return (S, E) => (_(), g("div", Wu, [
      ...
      r("span", {
        ...
        P(o(_u), {
          onDragover: E[0] || (E[0] = (A) => f(A)),
          onDragleave: E[1] || (E[1] = (A) => v(A)),
          onDrop: E[2] || (E[2] = (A) => u(A)),
          onClick: m,
      ...
      o(e).fs.loading ? (_(), g("span", {
        ...
        P(o(hu), {
          onClick: E[3] || (E[3] = (A) => o(e).emitter.emit("vf-fetch-abort"))
        })
      ], 8, Zu)) : (_(), g("span", {
        key: 0,
        title: o(n)("Refresh")
      ...
      _e(r("div", {
        ...
        r("div", null, [
          P(o($u), {
            onDragover: E[4] || (E[4] = (A) => f(A)),
            onDragleave: E[5] || (E[5] = (A) => v(A)),
            onDrop: E[6] || (E[6] = (A) => u(A, -1)),
            onClick: E[7] || (E[7] = (A) => o(e).emitter.emit("vf-fetch", { params: { q: "index", adapter: o(e).fs.adapter } }))
          })
        ]),
        r("div", Qu, [
          o(e).fs.hiddenBreadcrumbs.length ? _e((_(), g("div", e_, [
            ...
            r("div", n_, [
              r("span", {
                onDragenter: E[8] || (E[8] = (A) => o(e).fs.toggleHiddenBreadcrumbs(!0)),
                onClick: E[9] || (E[9] = (A) => o(e).fs.toggleHiddenBreadcrumbs()),
        ...
        r("div", {
          ...
          (_(!0), g(he, null, ge(o(e).fs.breadcrumbs, (A, C) => (_(), g("div", { key: C }, [
            s_,
            r("span", {
              onDragover: (x) => C === o(e).fs.breadcrumbs.length - 1 || f(x),
              onDragleave: (x) => C === o(e).fs.breadcrumbs.length - 1 || v(x),
              onDrop: (x) => C === o(e).fs.breadcrumbs.length - 1 || u(x, C),
              class: "vuefinder__breadcrumb__item",
              title: A.basename,
              onClick: (x) => o(e).emitter.emit("vf-fetch", { params: { q: "index", adapter: o(e).fs.adapter, path: A.path } })
            }, b(A.name), 41, o_)
          ]))), 128))
        ], 512),
        o(e).fs.loading ? (_(), K(o(is), { key: 0 })) : U("", !0)
      ...
      _e(r("div", r_, [
        ...
        _e(r("input", {
          ...
          onKeydown: $t(D, ["esc"]),
          onBlur: q,
          "onUpdate:modelValue": E[10] || (E[10] = (A) => tr(w) ? w.value = A : null),
      ...
      _e(r("div", l_, [
        (_(!0), g(he, null, ge(o(e).fs.hiddenBreadcrumbs, (A, C) => (_(), g("div", {
          key: C,
          onDragover: E[11] || (E[11] = (x) => f(x)),
          onDragleave: E[12] || (E[12] = (x) => v(x)),
          onDrop: (x) => l(x, C),
          onClick: (x) => h(A),
...
const Uo = { render: Ov }, Fv = ["data-type", "data-item", "data-index"], Cn = {
  ...
  setup(t) {
    const e = ae("ServiceContainer"), n = e.dragSelect, a = t, s = (m) => {
      ...
      mounted(m, h, $, y) {
        $.props.draggable && (m.addEventListener("dragstart", (V) => i(V, h.value)), m.addEventListener("dragover", (V) => l(V, h.value)), m.addEventListener("drop", (V) => d(V, h.value)));
      },
      beforeUnmount(m, h, $, y) {
  ...
  setup(t) {
    ...
    const l = yt({ active: !1, column: "", order: "" }), u = (v = !0) => {
      ...
      if (!v)
        return p;
      const $ = (y, V) => typeof y == "string" && typeof V == "string" ? y.toLowerCase().localeCompare(V.toLowerCase()) : y < V ? -1 : y > V ? 1 : 0;
      return l.active && (p = p.slice().sort((y, V) => $(y[m], V[m]) * h)), p;
    }, f = (v) => {
  ...
  setup(t) {
    ...
    const f = (v) => {
      ...
      i.active = !0, ct(() => {
        var w;
        const V = (w = a.value) == null ? void 0 : w.getBoundingClientRect();
        let O = (V == null ? void 0 : V.height) ?? 0, R = (V == null ? void 0 : V.width) ?? 0;
