<template>
  <div class="vuefinder__explorer">
    <div v-if="app.view === 'list' || searchQuery.length" class="vuefinder__explorer__header">
      <div @click="sortBy('basename')" class="vuefinder__explorer__header__name vf-sort-button">
        {{ t('Name') }} <SortIcon :direction="sort.order" v-show="sort.active && sort.column === 'basename'"/>
      </div>
      <div v-if="!searchQuery.length" @click="sortBy('file_size')" class="vuefinder__explorer__header__size vf-sort-button">
        {{ t('Size') }} <SortIcon :direction="sort.order" v-show="sort.active && sort.column === 'file_size'"/>
      </div>
      <div v-if="!searchQuery.length" @click="sortBy('last_modified')" class="vuefinder__explorer__header__date vf-sort-button">
        {{ t('Date') }} <SortIcon :direction="sort.order" v-show="sort.active && sort.column === 'last_modified'"/>
      </div>
      <div v-if="searchQuery.length" @click="sortBy('path')" class="vuefinder__explorer__header__path vf-sort-button">
    ...
    >
      ...
      <Item v-if="searchQuery.length" v-for="(item, index) in getItems()"
        ...
        <div class="vuefinder__explorer__search__item">
          <div class="vuefinder__explorer__search__item__name">
            <ItemIcon :type="item.type" :small="app.compactListView"/>
            <span class="vuefinder__explorer__search__item__name__text">{{ item.basename }}</span>
          </div>
          <div class="vuefidner__explorer__search__item__path">{{ item.path }}</div>
        </div>
      </Item>
      <!-- List View -->
      <Item v-if="app.view==='list' && !searchQuery.length" v-for="(item, index) in getItems()"
        ...
        <div class="vuefinder__explorer__item">
          <div class="vuefinder__explorer__item__name">
            <ItemIcon :type="item.type" :small="app.compactListView"/>
            <span class="vuefinder__explorer__item__name__text">{{ item.basename }}</span>
          </div>
          <div class="vuefinder__explorer__item__size">{{ item.file_size ? app.filesize(item.file_size) : '' }}</div>
          <div class="vuefinder__explorer__item__date">
      ...
      <Item v-if="app.view==='grid' && !searchQuery.length" v-for="(item, index) in getItems(false)"
        ...
        <div>
          <div class="relative">
            ...
            <div class="absolute vuefinder__explorer__item__grid__ext"
...
.vuefinder__explorer__header__name {
  @apply col-span-7;
}

.vuefinder__explorer__header__size {
  @apply justify-center col-span-2;
}

.vuefinder__explorer__header__date {
  @apply justify-center col-span-3;
}

.vuefinder__explorer__header__path {
...
.vuefinder__explorer__search__item__name__text {
...
.vuefinder__explorer__item__name {
  @apply flex col-span-7 items-center;
}

.vuefinder__explorer__item__name__text {
  @apply overflow-ellipsis overflow-hidden whitespace-nowrap;
}

.vuefinder__explorer__item__size {
  @apply col-span-2 text-center;
}

.vuefinder__explorer__item__date {
  @apply col-span-3 overflow-ellipsis overflow-hidden whitespace-nowrap px-1 md:px-3;
}

.vuefinder__explorer__item__grid__ext {
  @apply hidden md:block top-1/2 w-full text-center text-neutral-500;
}
