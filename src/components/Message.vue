<template>
  <div>
    <div
        v-if="!hidden"
        ref="strMessage" class="vuefinder__message"
        :class="error ? 'vuefinder__message--error ' : 'vuefinder__message--success'">
      <slot></slot>
      <div class="vuefinder__message__close" @click="hide"
      :title="t('Close')">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="vuefinder__message__close__icon">
...
.vuefinder__message__close__icon {
  @apply w-5 h-5;
}
</style>
