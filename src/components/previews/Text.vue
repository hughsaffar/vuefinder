<template>
  <div class="flex">
    <div class="vuefinder__text_preview_modal__title" id="modal-title"
         :title="app.modal.data.item.path">
      {{ app.modal.data.item.basename }}
    </div>
    <div class="vuefinder__text_preview_modal__buttons">
      <button @click="save" class="vuefinder__text_preview_modal__button--primary" v-if="showEdit">
        {{ t('Save') }}</button>
      <button class="vuefinder__text_preview_modal__button--secondary" @click="editMode()" v-if="app.features.includes(FEATURES.EDIT)">{{ showEdit ? t('Cancel'): t('Edit') }}</button>
    </div>
  </div>
  <div>
    <pre v-if="!showEdit" class="vuefinder__text_preview_modal__content">{{ content }}</pre>
    <div v-else>
      <textarea
          ref="editInput"
          v-model="contentTemp"
          class="vuefinder__text_preview_modal__textarea" name="text" id="" cols="30" rows="10"></textarea>
...
.vuefinder__text_preview_modal__buttons {
  @apply ml-auto mb-2;
}
